...
| D6  | Zod 入参校验 + 错误态 UI         | [ ]  |
| D7  | Demo A README + 对照 5.5 验收    | [ ]  |

**每日要点**

1. **D1**：按 2.4 初始化；跑通 Getting Started 流式对话
...
7. **D7**：写 Demo A README 草稿；对照 5.5 验收
...
1. **缺 tool result**：客户端 tool 必须 `addToolOutput`；阶段 2 还会再碰到
2. **流式不出字**：确认 `return result.toDataStreamResponse()`（以当前 SDK 文档为准）
